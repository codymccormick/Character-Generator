...
export const generateCaretaker = (type) => {
  ...
  export const generateParents = (maxChildAge) => {
	...
	const fatherEvent = fatherCaretakerStatus.status === "Death"
	  ? rollDeath()
	  : fatherCaretakerStatus.status === "Misfortune"
	  ? rollMisfortune()
	  : null;
  
	const motherEvent = motherCaretakerStatus.status === "Death"
	  ? rollDeath()
...
export const rollCaretakerStatus = () => {
	...
	if (roll <= 6) {
	  ...
	  return { status: "Alive and well", description: "Your parents or guardians are both doing well" };
	} else if (roll <= 10) {
	  // 33% chance of parents/guardians experiencing misfortune
	  return { status: "Misfortune", description: "Your parents or guardians have experienced misfortune" };
	} else {
